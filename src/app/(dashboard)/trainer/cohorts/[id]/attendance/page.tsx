      ...
      <div className="page-body">
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button className={`btn ${halfDay === 'AM' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setHalfDay('AM')}>Matin</button>
            <button className={`btn ${halfDay === 'PM' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setHalfDay('PM')}>Après-midi</button>
          </div>
          {loading && (
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Mise à jour...</span>
          )}
          {/* Stats */}
                      ...
                      return (
                        <div key={j.id} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--bg-secondary)' }}>
                            ...
                            <button className="btn btn-sm btn-danger" style={{ fontSize: 10 }} onClick={() => handleDeleteJust(j.id)}>Supprimer</button>
                          </div>
                          {/* Description */}
                          {j.description && (
