...
const REASON_TYPES = [
  { value: 'MEDICAL',   label: 'ðŸ¥ MÃ©dical' },
  { value: 'FAMILY',    label: 'ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ Familial' },
  { value: 'TRANSPORT', label: 'ðŸšŒ Transport' },
  { value: 'ADMIN',     label: 'ðŸ“‹ Administratif' },
  { value: 'OTHER',     label: 'ðŸ“Œ Autre' },
];
    ...
    <>
      ...
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">ðŸ“‹ Saisie de prÃ©sence â€” {cohort?.name}</h1>
          <p className="page-subtitle">{cohort?.program?.name} â€” {cohort?.program?.campus?.name}</p>
        </div>
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
          {saving ? <><span className="loading-spinner" style={{ width: 14, height: 14 }} /> Enregistrement...</> : saved ? 'âœ“ EnregistrÃ©' : 'ðŸ’¾ Enregistrer'}
        </button>
      </div>
      ...
      <div className="page-body">
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button className={`btn ${halfDay === 'AM' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setHalfDay('AM'); setLoading(true); }}>ðŸŒ… Matin</button>
            <button className={`btn ${halfDay === 'PM' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setHalfDay('PM'); setLoading(true); }}>ðŸŒ‡ AprÃ¨s-midi</button>
          </div>
          {/* Stats */}
              ...
              return (
                <div key={l.id} style={{
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                    <button className={`attendance-btn present ${rec.status === 'PRESENT' ? 'selected' : ''}`} onClick={() => updateStatus(l.id, 'PRESENT')}>PrÃ©sent</button>
                    <button className={`attendance-btn absent ${rec.status === 'ABSENT' ? 'selected' : ''}`} onClick={() => updateStatus(l.id, 'ABSENT')}>Absent</button>
                    <button className={`attendance-btn justified ${rec.status === 'JUSTIFIED_ABSENT' ? 'selected' : ''}`} onClick={() => updateStatus(l.id, 'JUSTIFIED_ABSENT')}>JustifiÃ©</button>
                  ...
                  {isAbsent && (
                    <button
                      ...
                      title="GÃ©rer les justificatifs"
                    >
                      ðŸ“Ž Justificatif{rec.status === 'JUSTIFIED_ABSENT' ? ' âœ“' : ''}
                    </button>
                  )}
                </div>
              );
        ...
        <div className="modal-overlay" onClick={() => setJustModal(null)}>
          <div className="modal" style={{ maxWidth: 640, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">ðŸ“Ž Justificatifs â€” {justModal.learnerName}</h2>
              <button className="btn btn-ghost btn-icon" onClick={() => setJustModal(null)}>âœ•</button>
            </div>
            ...
            <div className="modal-body">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {justList.map((j: any) => {
                      const statusCfg: Record<string, { color: string; bg: string; label: string }> = {
                        PENDING:  { color: '#92400e', bg: '#fef9c3', label: 'â³ En attente' },
                        APPROVED: { color: '#166534', bg: '#dcfce7', label: 'âœ… ApprouvÃ©' },
                        REJECTED: { color: '#991b1b', bg: '#fee2e2', label: 'âŒ RejetÃ©' },
                      };
                      ...
                      return (
                        <div key={j.id} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--bg-secondary)' }}>
                            ...
                            {j.status === 'PENDING' && (
                              <button className="btn btn-sm btn-primary" style={{ fontSize: 10 }} onClick={() => handleApproveJust(j.id)}>âœ… Approuver</button>
                            )}
                            <button className="btn btn-sm btn-danger" style={{ fontSize: 10 }} onClick={() => handleDeleteJust(j.id)}>ðŸ—‘ï¸</button>
                          </div>
                          {/* Description */}
                          {j.description && (
                          ...
                          <div style={{ padding: '10px 14px' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              <input className="form-input" style={{ fontSize: 11, padding: '3px 8px', flex: 2 }} placeholder="ðŸ”— Lien (Google Drive, Dropbox...)" value={nd.url}
              ...
              <div style={{ borderTop: justList.length > 0 ? '2px dashed var(--border)' : 'none', paddingTop: justList.length > 0 ? 20 : 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, color: 'var(--text-primary)' }}>
                  {justList.length === 0 ? 'ðŸ“‹ Ajouter un justificatif' : '+ Nouveau justificatif'}
                </div>
                <form onSubmit={handleCreateJust}>
                  ...
                  <div style={{ padding: '8px 12px', background: '#f0f9ff', borderRadius: 6, fontSize: 12, color: '#0369a1', marginBottom: 12 }}>
                    ðŸ“… Date automatiquement fixÃ©e au <strong>{new Date(date).toLocaleDateString('fr-FR', { weekday: 'long', day: '2-digit', month: 'long' })}</strong> â€” {halfDay === 'AM' ? 'ðŸŒ… Matin' : 'ðŸŒ‡ AprÃ¨s-midi'}
                  </div>
                  <div className="form-group">
                    <label className="form-label">Motif / Description</label>
                    <textarea className="form-input" rows={2} style={{ resize: 'vertical' }} placeholder="Certificat mÃ©dical, urgence familiale..."
                      value={justForm.description} onChange={e => setJustForm({ ...justForm, description: e.target.value })} />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                    <div className="form-group" style={{ margin: 0 }}>
                      <label className="form-label">ðŸ”— Lien piÃ¨ce justificative (optionnel)</label>
                    ...
                    <div className="form-group" style={{ margin: 0 }}>
                      ...
                      <input className="form-input" placeholder="Certificat mÃ©dical..." value={justForm.docName}
                  ...
                  <div style={{ marginTop: 12, padding: '8px 12px', background: '#fef9c3', borderRadius: 6, fontSize: 11, color: '#713f12' }}>
                    ðŸ’¡ L&apos;apprenant passera automatiquement en statut &quot;Absent justifiÃ©&quot; et le justificatif sera visible dans son profil.
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 14, gap: 8 }}>
                    ...
                    <button type="submit" className="btn btn-primary" disabled={savingJust}>
                      {savingJust ? 'â³ Enregistrement...' : 'ðŸ“‹ CrÃ©er le justificatif'}
                    </button>
                  </div>
                </form>
              </div>
